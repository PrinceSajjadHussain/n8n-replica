...
pnpm install
pnpm --filter @flowforge/api db:migrate   # safe to re-run; only applies what's missing
pnpm dev   # runs api + worker + web together (see root package.json)
```
Web UI: http://localhost:5173 · API: http://localhost:4000

Postgres/Redis published ports are `5433`/`6380` on the host (not the
default `5432`/`6379`) specifically so `docker compose up` doesn't collide
with a Postgres/Redis already running on your machine — `.env.example`
already points at those ports. Containers still talk to each other
internally on the normal `5432`/`6379`.

**Don't use `prisma migrate deploy` on a database that's ever had a
migration applied by hand (`psql -f migration.sql`)** — Prisma's own
migration-tracking table won't know about it and will try to replay
`00000000000000_init` from scratch, failing with "relation already
exists". `pnpm --filter @flowforge/api db:migrate` (backed by
`apps/api/src/scripts/migrate.ts`) is the one to use instead: it keeps its
own tracking table, auto-detects and skips migrations that are already
applied (by checking whether the tables/columns they create already
exist), and is always safe to run again — including from inside a running
container: `docker compose exec api npm run db:migrate`.
...
## 14. Step 7 — Enterprise & extensibility

- ✅ **SSO** — SAML, OIDC, and LDAP connections, configured at
  `/admin/sso` (requires `admin` system role). SAML/OIDC log in via
  redirect; LDAP posts username/password directly to
  `/auth/sso/:id/ldap-login`. All three auto-provision a FlowForge user on
  first successful login.
- ✅ **RBAC / audit log / API tokens** — an instance-wide `systemRole`
  (`member`/`admin`/`superadmin`) layered on top of per-workspace roles;
  the very first account created on an instance is auto-promoted to
  superadmin. Manage roles at `/admin/roles`, review activity at
  `/admin/audit-log`, issue scoped long-lived tokens at
  `/admin/api-tokens` (pass one as `Authorization: Bearer ffk_...`).
  Both session and token requests are rate-limited (Redis-backed, falls
  back to in-memory if `REDIS_URL` is unset).
- ✅ **Custom node SDK/CLI/hot-reload** — `@flowforge/node-sdk` +
  `@flowforge/node-cli` (`flowforge-node create|test|docs`) scaffold and
  test a node package locally; drop it in the worker's `CUSTOM_NODES_DIR`
  (default `./custom-nodes`) and it hot-reloads on save, no restart
  needed. Distinct from marketplace `CommunityNode` installs.
- ✅ **Import/export** — `POST /workflows/import` accepts an n8n, Make.com,
  or Zapier export and creates a FlowForge workflow (unmapped node types
  become `code` nodes with the original config preserved, plus
  warnings). `GET /workflows/:id/export/:target` exports to `langgraph`,
  `crewai`, `python`, or `docker` (a runnable `.tar.gz` bundle).
- ✅ **UI polish** — sticky notes, node grouping, auto-layout, mini-map,
  a command palette (⌘K/Ctrl+K) on the canvas, and a template gallery at
  `/templates`.

Applying this migration on an existing database: `pnpm --filter
@flowforge/api db:migrate` (see §0) — it only adds what's missing
(`SsoConnection`, `ApiToken`, `CustomNodePackage` tables, `User.systemRole`,
`ActivityLog.ipAddress`/`userAgent`) and is safe to run against a database
that already has everything through Step 6.
