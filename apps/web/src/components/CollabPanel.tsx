...
const TABS = ['Versions', 'Comments', 'Alerts', 'Activity', 'Settings'] as const;
  ...
  return (
    <div className="w-96 shrink-0 border-l border-panelBorder bg-panel flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-3">
        ...
        {tab === 'Settings' && <SettingsTab workflowId={workflowId} />}
      </div>
    </div>
  );
}
...
/** Error workflow — lets the user pick another workflow to run whenever
 *  this one fails. The picker only ever writes `errorWorkflowId`, so
 *  saving here can't clobber concurrent node/edge edits happening in the
 *  canvas (the PUT endpoint applies each field independently). */
function SettingsTab({ workflowId }: { workflowId: string }) {
  ...
  async function save(nextValue: string | null) {
    ...
    setSaved(null);
    try {
      await api.put(`/workflows/${workflowId}`, { errorWorkflowId: nextValue });
      setErrorWorkflowId(nextValue);
      setSaved('Saved.');
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="text-sm font-medium mb-1">Error workflow</p>
        <p className="text-xs text-muted mb-2">
          Runs automatically as its own execution whenever this workflow fails. It receives a trigger payload of{' '}
          <code className="text-signal">{'{ failedWorkflowId, executionId, errorMessage }'}</code> — read those
          fields from the trigger node's output (e.g. an <code className="text-signal">If</code> or{' '}
          <code className="text-signal">Slack</code> node) to report on or react to the failure. It does not receive
          the failed node's own input/output — look that up via the Executions API using{' '}
          <code className="text-signal">executionId</code> if you need it.
        </p>
        <select
        ...
        {saved && <p className="text-[11px] text-signal mt-1">{saved}</p>}
        {workflows.length === 0 && (
