    ...
    fields: [
      {
        ...
        options: [
          ...
          { value: 'gemini-1.5-flash-8b', label: 'Gemini 1.5 Flash-8B (smallest)' },
        ],
        help: 'Choose a Gemini model. 2.5 Pro is most capable; 2.0 Flash is the recommended default.',
      },
      {
        key: 'systemPrompt',
        label: 'System prompt',
        ...
        help: 'Optional system instruction sent before every user turn.',
      },
      {
        key: 'prompt',
        label: 'Prompt',
        ...
        help: 'User message. Use {{$trigger.message}} to always get the chatTrigger message from anywhere in the chain. Use {{$json.someField}} for output from the immediately upstream node.',
      },
      { key: 'temperature', label: 'Temperature', type: 'number', default: 0.3, min: 0, max: 2, step: 0.1, help: '0 = deterministic, 1 = balanced, 2 = creative.' },
      { key: 'maxOutputTokens', label: 'Max output tokens', type: 'number', default: 2048, min: 1, max: 65536, step: 1 },
      { key: 'jsonMode', label: 'JSON mode (output.parsed will contain the parsed object)', type: 'boolean', default: false },
    ],
  },
    ...
    fields: [
      {
        ...
        label: 'Session ID',
        type: 'expression',
        default: '{{$trigger.sessionId}}',
        placeholder: '{{$trigger.sessionId}}',
        help: 'Use {{$trigger.sessionId}} — resolves to the chatTrigger session ID from anywhere in the chain.',
      },
      {
        key: 'systemPrompt',
        label: 'System prompt',
        ...
        placeholder: 'You are a helpful assistant.',
        help: 'Instructions the AI follows for every turn.',
      },
      {
        key: 'prompt',
      ...
      {
        key: 'recentTurns',
        label: 'Recent turns to replay (short-term memory)',
        type: 'number',
        default: 12,
        min: 0,
        max: 100,
      ...
      {
        key: 'recallTopK',
        label: 'Top-K turns to recall',
        type: 'number',
        default: 4,
        min: 1,
        max: 20,
        step: 1,
        visibleIf: (p) => Boolean(p.longTermMemory),
      },
    ],
  },
  ...
  redisMemory: {
    fields: [
      ...
      },
      {
        ...
        default: '{{$trigger.sessionId}}',
        placeholder: '{{$trigger.sessionId}}',
        help: 'Use {{$trigger.sessionId}} — always resolves to the original chatTrigger sessionId regardless of how deep in the chain this node is. Required: leave blank and the node will error.',
      },
      {
        key: 'maxTurns',
