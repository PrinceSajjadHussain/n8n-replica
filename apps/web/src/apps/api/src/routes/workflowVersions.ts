...
workflowVersionsRouter.post('/:id/versions', async (req: AuthedRequest, res, next) => {
  try {
...
workflowVersionsRouter.get('/:id/versions', async (req: AuthedRequest, res, next) => {
  try {
    const userId = req.userId!;
    const workflowId = req.params.id;
    await assertOwnership(workflowId, userId);
...
/** Shared logic for publish + rollback (rollback is just "publish an older version"). */
async function publishVersion(workflowId: string, userId: string, version: number) {
  await assertOwnership(workflowId, userId);
  ...
  await pool.query(`UPDATE "WorkflowVersion" SET status = 'published' WHERE "workflowId" = $1 AND version = $2`, [
    workflowId,
    version,
  ]);
}
...
workflowVersionsRouter.post('/:id/versions/:version/publish', async (req: AuthedRequest, res, next) => {
  try {
    const userId = req.userId!;
    const workflowId = req.params.id;
    const version = Number(req.params.version);
    await publishVersion(workflowId, userId, version);
...
workflowVersionsRouter.post('/:id/versions/:version/rollback', async (req: AuthedRequest, res, next) => {
  try {
    const userId = req.userId!;
    const workflowId = req.params.id;
    const version = Number(req.params.version);
    await publishVersion(workflowId, userId, version);
    res.json({ published: true, version, rolledBack: true });
  } catch (err) {
    next(err);
  }
});
...
workflowVersionsRouter.get('/:id/versions/diff', async (req: AuthedRequest, res, next) => {
  try {
    const userId = req.userId!;
    const workflowId = req.params.id;
    await assertOwnership(workflowId, userId);
    ...
    const rows = await pool.query<{ version: number; nodesJson: unknown; edgesJson: unknown }>(
      `SELECT version, "nodesJson", "edgesJson" FROM "WorkflowVersion" WHERE "workflowId" = $1 AND version IN ($2, $3)`,
      [workflowId, from, to]
    );
    const fromRow = rows.rows.find((r) => r.version === from);
    const toRow = rows.rows.find((r) => r.version === to);
