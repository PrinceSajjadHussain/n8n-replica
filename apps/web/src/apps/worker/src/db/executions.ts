...
export async function getExecutionForRetry(
  ...
  const runsResult = await pool.query<{ nodeId: string; status: 'success' | 'failed' | 'skipped'; output: unknown; input: unknown }>(
    `SELECT "nodeId", status, output, input FROM "ExecutionNodeRun" WHERE "executionId" = $1`,
    [executionId]
  );
  return {
    workflowId: execution.workflowId,
    nodeRuns: runsResult.rows.map((r) => ({ nodeId: r.nodeId, status: r.status, output: r.output, input: r.input })),
  };
}
